...
class WebRTCConnectionGenerator {
  ...
  constructor(
    ...
    this.newPeerConnection.onicecandidate = (ev) =>
      this.handleICECandidateEvent(ev);
    this.newPeerConnection.oniceconnectionstatechange = () =>
      this.handleICEConnectionStateChangeEvent();
    this.newPeerConnection.onicegatheringstatechange = () =>
      this.handleICEGatheringStateChangeEvent();
    this.newPeerConnection.onsignalingstatechange = () =>
  ...
  closeVideoCall(): void {
    this.newPeerConnection
      .getTransceivers()
      .forEach((transceiver) => transceiver.receiver.track.stop());
  ...
  async handleVideoOfferMsg(msg: any): Promise<void> {
    ...
    if (this.newPeerConnection.signalingState !== "stable") {
      await Promise.all([
        ...
        this.newPeerConnection.setRemoteDescription(desc).catch(() => {
          alert("Ошибка подключения, попробуйте снова");
        }),
      ]);
    ...
    await this.newPeerConnection.setLocalDescription(
      await this.newPeerConnection.createAnswer()
    );
